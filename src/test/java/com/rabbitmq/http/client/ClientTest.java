...
public class ClientTest {
    ...
    protected Connection openConnection(String username, String password) throws IOException, TimeoutException {
        ConnectionFactory cf = new ConnectionFactory();
    ...
    protected Connection openConnection(String clientProvidedName) throws IOException, TimeoutException {
    ...
    protected static void verifyUpstreamDefinitions(String vhost, java.util.List<UpstreamInfo> upstreams, String upstreamName) {
        assertThat(upstreams).isNotEmpty();
        UpstreamInfo upstream = upstreams.stream()
    ...
    protected static boolean waitAtMostUntilTrue(int timeoutInSeconds, java.util.function.Supplier<Boolean> callback) {
        if (callback.get()) {
            return true;
        }
    ...
    void getApiOverview() throws Exception {
        ...
        java.util.List<String> xts = res.getExchangeTypes().stream()
    ...
    void getApiConnectionsUsernameWithName() throws Exception {
        ...
        java.util.List<UserConnectionInfo> xs = awaitEventPropagation(() -> client.getConnectionsOfUser(username));
    ...
    void getApiChannels() throws Exception {
        ...
        chs = chs.stream()
                .filter(chi -> chi.getName().startsWith(cn))
                .collect(Collectors.toList());
    ...
    void getApiChannelsWithPaging() throws Exception {
        ...
        com.rabbitmq.client.Channel originalChannel = channels.stream()
    ...
    void getApiExchangesWithPaging() {
        ...
        ExchangeInfo x = pagedXs.getItemsAsList().stream()
    ...
    void postApiExchangesWithVhostWithNamePublish() throws Exception {
        ...
        java.util.concurrent.atomic.AtomicReference<String> payloadReference = new java.util.concurrent.atomic.AtomicReference<>();
        java.util.concurrent.atomic.AtomicReference<com.rabbitmq.client.AMQP.BasicProperties> propertiesReference = new java.util.concurrent.atomic.AtomicReference<>();
        ...
        boolean routed = client.publish(v, "amq.direct", q,
        ...
        assertThat(propertiesReference.get().getHeaders().get("header1").toString()).isEqualTo("value1");
    ...
    void getApiExchangesWithVhostWithNameBindingsSource() throws Exception {
        ...
        BindingInfo x = xs.stream()
                .filter(b -> "".equals(b.getSource()) && 
    ...
    void getApiExchangesWithVhostWithNameBindingsDestination() throws Exception {
        ...
        BindingInfo x = xs.stream()
                .filter(b -> src.equals(b.getSource()) &&
    ...
    void getApiQueuesWithDetails() throws Exception {
        ...
        DetailsParameters detailsParameters = new DetailsParameters().messageRates(60, 5).lengths(60, 5);
        Supplier<QueueInfo> request = () -> client.getQueues(detailsParameters).stream()
                .filter(qi -> qi.getName().equals(q)).findFirst().orElse(null);
        waitAtMostUntilTrue(10, () -> {
            QueueInfo qi = request.get();
            return qi != null && qi.getMessagesDetails() != null && qi.getMessagesDetails().getRate() > 0;
        });
    ...
    void getApiQueuesWithPaging() throws Exception {
        ...
        QueryParameters queryParameters = new QueryParameters().name("^queue-for-paging-test-*", true)
                .pagination().pageSize(10).query();
    ...
    void getApiQueuesWithPagingAndNavigating() throws Exception {
        ...
        QueryParameters queryParameters = new QueryParameters().name("^queue-for-paging-and-navigating-test-*", true)
                .pagination().pageSize(10).query();
        Page<QueueInfo> page = client.getQueues(queryParameters);
    ...
    void getApiQueuesWithPagingAndDetails() throws Exception {
        ...
        executorService.submit(() -> {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ch.basicPublish("amq.fanout", "", null, "".getBytes(java.nio.charset.StandardCharsets.UTF_8));
                    Thread.sleep(10);
                } catch (Exception e) {
                    return;
                }
        ...
        QueryParameters queryParameters = new DetailsParameters().messageRates(60, 5).lengths(60, 5)
                .queryParameters().name("^queue-for-paging-and-details-test-*", true).pagination().pageSize(10).query();
        Supplier<Page<QueueInfo>> request = () -> client.getQueues(queryParameters);
        waitAtMostUntilTrue(10, () -> {
            Page<QueueInfo> p = request.get();
            return p.getFilteredCount() == queues.size() && p.getItems().length > 0 &&
                    p.getItems()[0].getMessagesDetails() != null && p.getItems()[0].getMessagesDetails().getRate() > 0;
        });
    ...
    void getApiQueuesWithNameWithDetails() throws Exception {
        ...
        Supplier<QueueInfo> request = () -> client.getQueue("/", q, new DetailsParameters().messageRates(60, 5).lengths(60, 5));
        waitAtMostUntilTrue(10, () -> {
            QueueInfo qi = request.get();
    ...
    void getApiConsumers() throws Exception {
        ...
        ConsumerDetails cons = cs.stream().filter(c -> c.getConsumerTag().equals(consumerTag)).findFirst().orElse(null);
    ...
    void getApiBindings() throws Exception {
        ...
        long count = xs.stream().filter(b -> DestinationType.QUEUE.equals(b.getDestinationType()) && x.equals(b.getSource())).count();
    ...
    void getApiBindingsWithVhostExample2() throws Exception {
        ...
        List<BindingInfo> xs = client.getBindings("/");
        BindingInfo b = xs.stream().filter(bi -> DestinationType.QUEUE.equals(bi.getDestinationType()) && x.equals(bi.getSource()) && q.equals(bi.getDestination())).findFirst().orElse(null);
    ...
    void postApiQueuesWithVhostWithExchangeGet() throws Exception {
        ...
        com.rabbitmq.client.AMQP.BasicProperties properties = new com.rabbitmq.client.AMQP.BasicProperties.Builder()
                .contentType("text/plain").deliveryMode(1).priority(5).headers(java.util.Collections.singletonMap("header1", "value1")).build();
        for (int i = 1; i <= messageCount; i++) {
            ch.basicPublish("", q, properties, ("payload" + i).getBytes(java.nio.charset.Charset.forName("UTF-8")));
        }
        ch.waitForConfirms(5_000);
        List<InboundMessage> messages = client.get(v, q, messageCount, GetAckMode.NACK_REQUEUE_TRUE, GetEncoding.AUTO, -1);
    ...
    @org.junit.jupiter.params.provider.ValueSource(strings = {
            "http-created", "http-created2", "http_created", "http created",
            "создан по хатэтэпэ", "creado a través de HTTP", "通过http",
            "HTTP를 통해 생성", "HTTPを介して作成", "created over http?",
            "created @ http API", "erstellt über http", "http पर बनाया",
    ...
    void getApiVhostsWithNamePermissionsWhenVhostExists() {
        ...
        UserPermissions x = xs.stream().filter(up -> "guest".equals(up.getUser())).findFirst().orElse(null);
    ...
    void getApiVhostsWithNameTopicPermissionsWhenVhostExists() {
        ...
        TopicPermissions x = xs.stream().filter(tp -> "guest".equals(tp.getUser())).findFirst().orElse(null);
    ...
    void getApiUsersWithNamePermissionsWhenUserExists() {
        ...
        UserPermissions x = xs.stream().filter(up -> "/".equals(up.getVhost())).findFirst().orElse(null);
        assertThat(x.getRead()).isEqualTo(".*");
    }
    ...
    void getApiUsersWithNameTopicPermissionsWhenUserExists() {
        ...
        TopicPermissions x = xs.stream().filter(tp -> "/".equals(tp.getVhost())).findFirst().orElse(null);
        assertThat(x.getExchange()).isEqualTo("amq.topic");
        assertThat(x.getRead()).isEqualTo(".*");
    }
    ...
    void putApiUsersWithNameWithBlankPasswordHash() throws Exception {
        ...
        String h = "";
        client.deleteUser(u);
        client.createUserWithPasswordHash(u, h.toCharArray(), java.util.Arrays.asList("original", "management"));
        client.updatePermissions("/", u, new UserPermissions(".*", ".*", ".*"));
        try {
            openConnection("alt-user", "alt-user");
            org.junit.jupiter.api.Assertions.fail("Expected exception");
        } catch (Exception e) {
            assertThat(e instanceof com.rabbitmq.client.AuthenticationFailureException || e instanceof IOException).isTrue();
        }
        client.deleteUser(u);
    }
    ...
    void getApiPermissions() {
        ...
        UserPermissions x = xs.stream().filter(up -> "/".equals(up.getVhost()) && s.equals(up.getUser())).findFirst().orElse(null);
        assertThat(x.getRead()).isEqualTo(".*");
    }
    ...
    void putApiPermissionsWithVhostWithUserWhenBothUserAndVhostExist() {
        ...
        String u = "hop-user1";
        client.createUser(u, "test".toCharArray(), java.util.Arrays.asList("management", "http", "policymaker"));
    ...
    void getApiTopicPermissions() {
        ...
        TopicPermissions x = xs.stream().filter(tp -> "/".equals(tp.getVhost()) && s.equals(tp.getUser())).findFirst().orElse(null);
        assertThat(x.getExchange()).isEqualTo("amq.topic");
        assertThat(x.getRead()).isEqualTo(".*");
    }
    ...
    void getApiTopicPermissionsWithVhostWithUserWhenBothVhostAndUserExist() {
        ...
        TopicPermissions x = xs.stream().filter(tp -> v.equals(tp.getVhost()) && u.equals(tp.getUser())).findFirst().orElse(null);
        assertThat(x.getExchange()).isEqualTo("amq.topic");
        assertThat(x.getRead()).isEqualTo(".*");
    }
    ...
    void getApiExtensions() {
        ...
        List<java.util.Map<String, Object>> xs = (List<java.util.Map<String, Object>>) (List<?>) client.getExtensions();
        assertThat(xs).isNotEmpty();
    }
    ...
    void getApiDefinitionsQueues() {
        ...
        QueueInfo q = d.getQueues().stream().filter(qi -> "queue1".equals(qi.getName()) && "/".equals(qi.getVhost())).findFirst().orElse(null);
    ...
    void getApiDefinitionsExchanges() {
        ...
        ExchangeInfo e = d.getExchanges().stream().filter(ex -> "exchange1".equals(ex.getName())).findFirst().orElse(null);
    ...
    void getApiDefinitionsBindings() {
        ...
        BindingInfo b = d.getBindings().stream().filter(bi ->
                "amq.fanout".equals(bi.getSource()) && "queue1".equals(bi.getDestination()) && DestinationType.QUEUE.equals(bi.getDestinationType())
    ...
    void getApiDefinitionsParameters() {
        ...
        RuntimeParameter<Object> parameter = parameters.stream().filter(p -> "shovel1".equals(p.getName())).findFirst().orElse(null);
    ...
    void getApiParametersShovel() {
        ...
        ShovelInfo s = shovels.stream().filter(sh -> "shovel1".equals(sh.getName())).findFirst().orElse(null);
    ...
    void getApiParametersShovelWithMultipleUris() {
        ShovelDetails value = new ShovelDetails(
                java.util.Arrays.asList("amqp://localhost:5672/vh1", "amqp://localhost:5672/vh3"),
                java.util.Arrays.asList("amqp://localhost:5672/vh2", "amqp://localhost:5672/vh4"), 30, true, null);
        value.setSourceQueue("queue1");
        value.setDestinationExchange("exchange1");
        client.declareShovel("/", new ShovelInfo("shovel2", value));
        List<ShovelInfo> shovels = awaitEventPropagation(() -> client.getShovels());
        ShovelInfo s = shovels.stream().filter(sh -> "shovel2".equals(sh.getName())).findFirst().orElse(null);
        assertThat(s).isNotNull();
    ...
    void putApiParametersShovelWithAnEmptyPublishPropertiesMap() {
        ShovelDetails value = new ShovelDetails("amqp://localhost:5672/vh1", "amqp://localhost:5672/vh2", 30, true, java.util.Collections.emptyMap());
        value.setSourceQueue("queue1");
        value.setDestinationExchange("exchange1");
        try {
        ...
        try { client.deleteShovel("/", "shovel1"); } catch (Exception e) {}
        try { client.deleteQueue("/", "queue1"); } catch (Exception e) {}
    }
    ...
    void getApiShovels() {
        ...
        assertThat(shovels).isNotEmpty();
        ShovelStatus s = shovels.stream().filter(sh -> shovelName.equals(sh.getName())).findFirst().orElse(null);
        assertThat(s).isNotNull();
        assertThat(s.getType()).isEqualTo("dynamic");
        waitAtMostUntilTrue(30, () -> {
            ShovelStatus status = client.getShovelsStatus().stream().filter(sh -> shovelName.equals(sh.getName())).findFirst().orElse(null);
    ...
    void getApiParametersFederationUpstreamDeclareAndGetAtRootVhostWithNonNullAckMode() {
        ...
        UpstreamInfo upstream = upstreams.stream().filter(u -> upstreamName.equals(u.getName())).findFirst().orElse(null);
    ...
    void getApiParametersFederationUpstreamSetDeclareAndGet() {
        ...
        UpstreamSetInfo upstreamSet = upstreamSets.stream().filter(u -> upstreamSetName.equals(u.getName())).findFirst().orElse(null);
    ...
    void getApiVhostLimits() {
        ...
        VhostLimits limits1 = limits.stream().filter(l -> vhost1.equals(l.getVhost())).findFirst().orElse(null);
    ...
    void getApiVhostLimitsWithoutLimitsOnAnyHost() {
        ...
        VhostLimits limits1 = limits.stream().filter(l -> "/".equals(l.getVhost())).findFirst().orElse(null);
